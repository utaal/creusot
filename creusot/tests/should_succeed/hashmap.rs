...
trait Hash: DeepModel {
    ...
    #[logic]
    #[ensures(result >= 0 && result <= @u64::MAX)]
...
impl<K: Hash, V> MyHashMap<K, V> {
    ...
    fn bucket_ix(self, k: K::DeepModelTy) -> Int {
        pearlite! { K::hash_log(k) % (@self.buckets).len() }
    }
}
...
impl<K: Hash + Copy + Eq + DeepModel, V: Copy> MyHashMap<K, V> {
    ...
    #[requires((*self).hashmap_inv())]
    #[ensures((^self).hashmap_inv())]
    #[ensures(forall<k : K> (@^self).get(k.deep_model()) == (@self).get(k.deep_model()))] // lets prove the extensional version for now
    #[allow(dead_code)]
    fn resize(&mut self) {
        let old_self = ghost! { self };
